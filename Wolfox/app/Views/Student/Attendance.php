     ...
     <div class="page-wraper">
         ...
         <div class="page-content bottom-content ">
             ...
             <div class="container mt-4">
                 ...
                 <?php
                    $session = session();
                    $durationMonths = filter_var($session->get('duration'), FILTER_SANITIZE_NUMBER_INT);
                    $joining = strtotime($session->get('joiningdate'));
                    $joiningDay = date('d', $joining);
                    $sid = $session->get('sid'); // Assuming student ID is stored in session

                    // Convert duration into total days
                    $totalDays = 30 * $durationMonths; // Approximate month length
                    $remainingDays = $totalDays;

                    $currentDateObj = new DateTime(date('Y-m-d', $joining));

                    // Fetch existing attendance records from DB
                    $db = \Config\Database::connect();
                    $attendanceData = [];
                    $query = $db->query("SELECT absent_dates FROM attendance WHERE sid = ?", [$sid]);
                    foreach ($query->getResult() as $row) {
                        $attendanceData[] = $row->absent_date;
                    }

                    while ($remainingDays > 0) {
                        $month = $currentDateObj->format('m');
                        $year = $currentDateObj->format('Y');
                        $daysInMonth = cal_days_in_month(CAL_GREGORIAN, $month, $year);

                        // Start from joining day if it's the first month, otherwise from 1st
                        $startDay = ($currentDateObj->format('d') == $joiningDay) ? $joiningDay : 1;

                        // If it's the last month, limit the days displayed
                        if ($remainingDays < $daysInMonth) {
                            $daysInMonth = $startDay + $remainingDays - 1;
                        }

                        // Reduce remaining days
                        $remainingDays -= ($daysInMonth - $startDay + 1);
                    ?>
                     <form method="post" action="<?= base_url('/Student/attendancesave') ?>">
                         <input type="hidden" name="month" value="<?= $month ?>">
                         <input type="hidden" name="year" value="<?= $year ?>">

                         <div class="card mb-3">
                             <div class="card-header text-center font-weight-bold bg-primary text-white" style="background-color: #0093dd !important;">
                                 <?= date('F Y', strtotime("$year-$month-01")); ?>
                             </div>

                             <div class="card-body">
                                 <button type="button" class="btn btn-primary mb-3 selectAll"style="background-color: #0093dd !important;">Select All</button>
                                 <button type="button" class="btn btn-danger mb-3 deselectAll">Deselect All</button>
                                 <div class="row">
                                     <?php
                                        for ($day = $startDay; $day <= $daysInMonth && $remainingDays >= 0; $day++) {
                                            $fullDate = sprintf('%04d-%02d-%02d', $year, $month, $day);
                                            $dayOfWeek = date('l', strtotime($fullDate));
                                            $isSunday = ($dayOfWeek == 'Sunday');
                                            $textColor = $isSunday ? 'text-danger' : '';
                                            $disabled = $isSunday ? 'disabled' : '';

                                            // If date exists in DB, make checkbox checked and disabled
                                            $isChecked = in_array($fullDate, $attendanceData) ? 'checked disabled' : '';
                                        ?>
                                         <div class="col-md-3 col-sm-4 col-6 mb-2">
                                             <div class="form-check">
                                                 <input class="form-check-input absent-checkbox" type="checkbox" name="absent_days[]" value="<?= $fullDate ?>" id="day_<?= $fullDate ?>" <?= $disabled ?> <?= $isChecked ?>>
                                                 <label class="form-check-label <?= $textColor ?>" for="day_<?= $fullDate ?>">
                                                     <?= $fullDate ?> (<?= $dayOfWeek ?>)
                                                 </label>
                                             </div>
                                         </div>
                             ...
                             <script>
                                 document.querySelectorAll(".selectAll").forEach(button => {
                                     button.addEventListener("click", function() {
                                         // Find the closest card-body (i.e., section of the current month)
                                         let cardBody = this.closest(".card-body");
                                         let checkboxes = cardBody.querySelectorAll(".absent-checkbox:not(:disabled)");

                                         // Check all checkboxes inside this section
                                         checkboxes.forEach(checkbox => checkbox.checked = true);
                                     });
                                 });

                                 document.querySelectorAll(".deselectAll").forEach(button => {
                                     button.addEventListener("click", function() {
                                         // Find the closest card-body (i.e., section of the current month)
                                         let cardBody = this.closest(".card-body");
                                         let checkboxes = cardBody.querySelectorAll(".absent-checkbox:not(:disabled)");
                 ...
                 <?php
                        // Move to the next month and set day to 1 to start from the first day
                        $currentDateObj->modify('first day of next month');
                    } ?>
             </div>
