 ...
 <?php
    ...
    //  print_r($feestudent);exit;
    $totalfee = $feestudent['totalfee'];
    $paidfee = $feestudent['paidfee'];
    //$remainingfee = $student['remainingfee'];
    $installment_number = $feestudent['installment_number'];
    ...
    $confirmstatus = $student['confirmstatus'];
         ...
         <div class="container">
             <div class="header-content">
                 ...
                 <div class="mid-content">
                     ...
                     <h5 class="mb-0" style="color: #0093dd;">Fee Document List</h5>
 ...
 <div class="container">
     <?php if ($remainingfee != 0) { ?>
         <div style="text-align: end; margin: 10px;">
             <a class="btn btn-submit" id="addDocumentBtn" href="#" style="border: 1px solid black; padding: 10px;border:none; background-color: #0093dd;transition: 0.3s;color: white;">Add Payment Screenshot</a>
         </div>
     <?php } ?>
     <div class="table-responsive">
         <table class="table table-bordered table-striped">
             <thead class="thead-dark">
                 <?php if ($confirmstatus == 1 && $installment != '') { ?>
                     <tr>
                         <th>Sr.No</th>
                         <th>Action</th>
                         <th>Total Fee</th>
             ...
             <tbody>
                 <?php if (!empty($fees)) : ?>
                     ...
                     <?php foreach ($fees as $fee) :
                            if ($confirmstatus == 1 && $installment != '' && $fee['installment_number'] != $installment_number + 1) {
                        ?>
                             <tr>
                                 <td><?= $i++; ?></td>
                                 <td>
                                     <?php if ($fee['mode'] === 'cash' && $fee['confirmstatus'] == 1) : ?>
                                         <div class="d-flex justify-content-center gap-4 mb-3">
                                             <a href="<?= base_url('/Student/feereceipt/' . $fee['id']) ?>" target="_blank" style="margin-top: 25px;" id="joiningBtn">
                                                 <i class="fa-solid fa-eye fa-lg text-primary p-2 rounded-circle shadow-sm action-icon pulse-effect openModalBtn" style="cursor: pointer; background: #eaf3ff;"></i>
                                             </a>
                                         </div>
                                     <?php elseif ($fee['mode'] === 'online') : ?>
                                         <div class="d-flex justify-content-center gap-4 mb-3">
                                             <?php if ($fee['confirmstatus'] == 1 && $fee['installment'] > 0) { ?>
                                                 <a href="<?= base_url('/Student/feereceipt/' . $fee['id']) ?>" target="_blank" style="margin-top: 25px;" id="joiningBtn">
                                                     <i class="fa-solid fa-eye fa-lg text-primary p-2 rounded-circle shadow-sm action-icon pulse-effect openModalBtn" style="cursor: pointer; background: #eaf3ff;"></i>
                                                 </a>
                                             <?php } elseif ($fee['confirmstatus'] == 2) { ?>
                                                 <a style="margin-top: 25px;" href="<?= base_url('/student/feedocument2/' . $fee['id']) ?>">
                                                     <i class="fa-solid fa-edit fa-lg text-warning p-2 rounded-circle shadow-sm action-icon" style="cursor: pointer; background: #fff5e6;"></i>
                                                 </a>
                                             <?php } ?>
                                         </div>
                                     <?php endif; ?>
                                 </td>
                                 <td><?= ucfirst($fee['totalfee']); ?></td>
                                 <td><?= ucfirst($fee['paidfee']); ?></td>
                                 <td><?= ucfirst($fee['mode']); ?></td>
                                 <td><?= date('d-m-Y', strtotime($fee['cashdate'])); ?></td>
                             </tr>
                     <?php }
                        endforeach;?> 
                 <?php else : ?>
                     <tr>
                        <p style="font-size: 25px;text-align:center;">No fee records found.</p> 
                     </tr>
                 <?php endif; ?>
             </tbody>
         </table>
 ...
 <script>
     ...
     if (rows.length > 0) {
         ...
         const lastFeeId = lastRow.querySelector('td:first-child').innerText;
         console.log(lastFeeId);
     ...
     addButton.href = `<?= base_url('/Student/feedocument/') ?>${feeId}`;
 </script>
