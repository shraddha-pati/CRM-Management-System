 <body>
     <div class="page-wraper">
         ...
         <div class="page-content bottom-content ">
                     ...
                     <table class="table table-bordered">
                         ...
                         <tbody>
                             <?php
                                ...
                                // $currentYear = date('Y', strtotime($joining));

                                $currentYear =  date('Y');
                                $joiningYear = date('Y', strtotime($joining));
                                $currentMonth = $MonthId;
                                $joiningDay =  date('d', strtotime($joining));
                                $daysInMonth  = cal_days_in_month(CAL_GREGORIAN, $currentMonth, $currentYear);

                                $loop = $daysInMonth - $joiningDay;


                                print_r($MonthId); 

                                // if ($MonthId == date('m', strtotime($joining))) {
                                    // for ($day = 1; $day <= $loop; $day++) {

                                    $date = sprintf('%04d-%02d', $joiningYear, $currentMonth);

                           ?>
                     <tr>
                         <td><input type="text" value="<?php echo $date; ?>" class="form-control" readonly></td>
                         <td>
                             <!-- <input type="text" name="attendance_status[<?php echo $date; ?>][status]" placeholder="Status" class="form-control" /> -->
                             <select name="Status" id="Status">
                                 <option value="0" class="form-control">Select Attendance Status</option>

                                 <option value="P" class="form-control">Present</option>
                                 <option value="A" class="form-control">Absent</option>
                                 <option value="CH" class="form-control">Company Holiday</option>
                             </select>
                             <!-- <textarea name="attendance_status[<?php echo $date; ?>][notes]" placeholder="Notes" class="form-control" rows="2"></textarea> -->
                         </td>
                     </tr>
                     </form>
             ...
             <button type="submit" class="btn btn-primary form-control">Submit Attendance</button>
             </form>
             </div>
         </div>
         <!-- Page Content End-->
